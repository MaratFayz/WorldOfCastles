 ...
 */
public class Form38 implements FormInterface 
{    
    String answer;
    EventKeeper EK;
    ArrayList<String> reqsymbols = new ArrayList(Arrays.asList("A"));
    
    private static final Logger logger = LogManager.getLogger(Form38.class.getName());
    ...
    public void Show()
    {
        ...
        {
              //NONE
              logger.log(Level.DEBUG, "Form38 выведена; ошибок не найдено");
        }
        else
        {
              ...
              logger.log(Level.DEBUG, "Form38 выведена; найдена ошибка");
        }                
    }
    ...
    public void CreateEvent()
    {
        ...
        logger.log(Level.DEBUG, "(из Form38) Был возвращен: this.EK: значение {}", this.EK);

        if(GlobalVariable.MissionLoader.isFileLoaded())
        {
            this.EK.PutNewEventInArray(new Command("Form38","NEXT","NULL","NULL"));
            logger.log(Level.DEBUG, "(из Form38) Была выполнена this.EK.PutNewEventInArray() и добавлена команда Command('Form38','NEXT','NULL','NULL')");
            logger.log(Level.DEBUG, "Form38 выведена; ошибок не найдено");
        }
        else
        {
            logger.log(Level.DEBUG, "Form38 выведена; ошибка найдена");
            logger.log(Level.DEBUG, "(из Form38) Нужно ввести значение по требованиям this.reqsymbols: {}", this.reqsymbols);
            this.answer = WaitAnswer(this.reqsymbols);

            if (this.answer.equals("A"))
            {
                this.EK.PutNewEventInArray(new Command("Form38","BACK","NULL","NULL"));
                logger.log(Level.DEBUG, "(из Form38) Была выполнена this.EK.PutNewEventInArray() и добавлена команда Command('Form38','BACK','NULL','NULL')");
            }
        }                    
    }
}
